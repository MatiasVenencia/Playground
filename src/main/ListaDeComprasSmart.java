...
import static main.ListaDeComprasSmart.Opcion.*;

//TODO: importar las clases necesarias para almacenar los productos.

/**
* La clase {@code ListaDeComprasSmart} es una versión mejorada de {@code ListaDeCompras} que permite controlar cantidad.
...
* @version 1.0
*/
public class ListaDeComprasSmart {
	...
	*/
	public static enum Opcion {
		AGREGAR {
			...
			public String descripcion() {
				return  opcion() + " <producto> [" + simboloOpcion() + "<cantidad>]\n" +
					"Agrega un nuevo producto a la lista de compras, opcionalmente se puede especificar la cantidad de este producto.";
			}
	...
	//TODO: agregue una lista global para almacenar los productos, considere que debe tener una cantidad asociada a cada producto.
	...
	* A diferencia de {@code ListaDeCompras}, esta aplicación tiene en cuenta la cantidad de cada producto, por ejemplo:
	* <pre>
	*   :agregar Shampoo
	*   :agregar Shampoo 3
        *   :mostrar
	...
	*/
	...
	private static boolean responderAEntrada(String entrada) {
		...
		if (comando.compareToIgnoreCase(AGREGAR.opcion()) == 0) {
			...
			String[] productoEnPartes = producto.split(Opcion.simboloOpcion(), 2);
			producto = productoEnPartes[0];
			int cantidad = 1;
			if (productoEnPartes.length > 1) { //el producto contiene un :algo, ese algo debería ser un número entero
				String cantidadCadena = productoEnPartes[1];
				if (cantidadCadena.trim().isEmpty()) {
					System.err.println("No hay una cantidad definida");
					return false;
				}
				try {
					cantidad = Integer.parseInt(cantidadCadena);
				} catch (NumberFormatException nfe) {
					System.err.println("Formato incorrecto para la cantidad, debe ser un entero");
